...
| Live deployment (sgc) | `files.sgc.ai`, alias `files.sgc.ai` on the prod box (shared Authentik via `auth.sgc.ai`, client `sgc-files`, groups `sgc-files-admins`/`sgc-files-editors`/`sgc-files-users`; mesh-DNS + `ipAllowList` 169.254.0.0/24 network gate + `userGroups` login gate — reference for locking a shared-IdP realm down to mesh+realm) | `1.5.0-2` |
